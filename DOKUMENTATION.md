...
| Mikromanagement-Schalter im Baufeld (`DPS-0.8.1.3`) | Kleine Ergänzung zu `DPS-0.8.1.2` auf ausdrücklichen Wunsch: Auftragstaktik (Zugführer befiehlt, Gruppenführer baut) soll der Normalfall bleiben, der Zugführer aber jederzeit selbst eingreifen können - genau die reale Spannung, die Auftragstaktik als Führungsprinzip bewusst einschränkt. Neuer Schalter "Mikromanagement" in `Baufeld.tsx` (→ `ui.baufeld.befehl`), sichtbar sobald mindestens ein Gruppenführer in der Sitzung sitzt - reine Client-Vorliebe (`useState`, kein Sync-Feld, kein neues Reducer-Feld), startet immer auf "Aus". Aktiviert baut der Zugführer trotz vorhandenem Gruppenführer direkt selbst (derselbe Pfad wie der bisherige Kein-Gruppenführer-Rückfall) - kein Befehl, keine Benachrichtigung. Deaktiviert bleibt exakt das aus `DPS-0.8.1.2` bekannte Verhalten unangetastet. Live mit drei echten Clients verifiziert: Schalter aktiviert → Zelt entsteht direkt beim Zugführer, Gruppenführer bekommt keine Benachrichtigung; Schalter deaktiviert → zweites Zelt löst wie gehabt einen Befehl mit Benachrichtigung beim Gruppenführer aus, keine Konsolenfehler. |
...
| `stil.einsatzleiste` | [`src/index.css:4638`](src/index.css#L4638) | Die angeheftete Leiste so flach wie möglich |
...
| `stil.hover` | [`src/index.css:4382`](src/index.css#L4382) | Hover nur mit echtem Zeiger - sonst klebt der Zustand |
...
| `stil.telefon` | [`src/index.css:4710`](src/index.css#L4710) | Anpassungen unter 760 px, inklusive Tabellenumbruch |
| `stil.tokens` | [`src/index.css:6`](src/index.css#L6) | Farben, Radien und Schatten der gesamten Oberfläche |
| `stil.touch` | [`src/index.css:4841`](src/index.css#L4841) | Mindestgroesse der Tippziele auf Touch-Geraeten |
...
| Branch | Stand |
| --- | --- |
| `DPS-0.8.1.3` | Mikromanagement-Schalter im Baufeld: der Zugführer kann trotz vorhandenem Gruppenführer per Client-seitigem Schalter (→ `ui.baufeld.befehl`) jederzeit selbst statt per Befehl bauen - Auftragstaktik bleibt Normalfall (Schalter startet immer "Aus"), Mikromanagement eine bewusste Ausnahme, kein neues State-/Reducer-Feld |
