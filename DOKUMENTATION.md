...
| Sprechfunk | Echte Live-Sprachverbindung statt Text (→ `ui.sprechfunk`), organisiert in frei wählbaren Rufgruppen (Kanal 1-3, Führung). Wer einen Kanal wählt, verbindet sich per WebRTC direkt (Mesh, kein eigener Medienserver) mit jeder anderen Person auf demselben Kanal - echtes, bidirektionales Gespräch für alle Beteiligten, nicht nur zur Übungsleitung. Eine Sprechen-Umschalttaste hält das Mikrofon standardmäßig stumm, wie bei einem echten Funkgerät. Signalisierung (Verbindungsaushandlung) läuft über denselben Transport wie alles andere, aber am Reducer vorbei - reine Zustellung zwischen zwei Personen, kein Spielzustand. Kein eigener TURN-Server vorhanden (nur öffentliches STUN) - kann in restriktiven Netzen (symmetrisches NAT, manche Schul-/Firmennetze) einzelne Verbindungen verhindern, offen dokumentiert statt verschwiegen. Ein gescheiterter eigener Mikrofonzugriff (Berechtigung verweigert, keine Hardware, keine Erlaubnis in einer eingebetteten Vorschau) blockiert die Verbindung nicht mehr komplett - die betroffene Person sieht eine klare Fehlermeldung und bleibt trotzdem empfangsbereit, nur das eigene Senden bleibt aus. Ersetzt den kurzlebigen Text-Funkkanal aus `DPS-0.6` vollständig |
...
| `state.sprechfunk` | [`src/state/useSprechfunk.ts:25`](src/state/useSprechfunk.ts#L25) | WebRTC-Mesh für einen gewählten Rufgruppen-Kanal |
