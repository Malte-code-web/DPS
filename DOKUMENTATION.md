...
| Private Spieler-Statusansicht "Mein Einsatz" (`DPS-0.8.0.15`) | Letzter offener Punkt aus "Fundament, Teil 2": eine neue, private Debriefing-Sektion "Mein Einsatz" zeigt jeder Person nur ihre eigenen Handlungen - auf Wunsch, damit ein Spieler nachvollziehen kann, was er genau gemacht hat. Da der Reducer bislang bei Maßnahme/Diagnostik/Sichtung/Verlegung keine Spieler-Zuordnung führte (nur teambasierte Aktionen wie Kollegenanfrage/Delegation/Rettung kannten schon eine Spieler-ID), war dafür eine echte Reducer-Erweiterung nötig statt nur einer UI-Änderung. Neues Feld `state.spielerProtokoll: SpielerProtokollEintrag[]` (spielerId, patientId, zeitSek, text), gefüllt über zwei neue Helfer: `protokolliereSpieler` für explizite Texte (Rettungskette) und `uebernimmVerlaufInSpielerprotokoll`, der den ohnehin schon an `patient.verlauf` angehängten Text unverändert übernimmt (`diagnostikDurchfuehren`, `patientSichten`, `massnahmeDurchfuehren`, `patientVerlegen`) statt ihn zu duplizieren - erkennt über einen Längenvergleich von `verlauf` vorher/nachher, ob überhaupt etwas passiert ist (kein Eintrag bei bereits durchgeführter Diagnostik). Team-Aktionen kreditieren alle Beteiligten mit derselben Zeile: eine abgeschlossene Notfallnarkose alle drei (anfragende Person + beide Team-Mitglieder), eine abgeschlossene Rettung anfragende Person und alle Helfer:innen. Die fünf betroffenen Aktionen bekamen dafür ein optionales `spielerId`-Feld (`state.sitzung.eigeneId`, im Einzelspiel `undefined` - dort protokolliert der Reducer bewusst nichts, die Haupttabelle deckt dort ohnehin schon alles ab), das an allen acht UI-Dispatch-Stellen (`Patientenansicht.tsx`, `Anhaengekarte.tsx`, `Verlegung.tsx`, `PatientKarte.tsx`, `RettungPanel.tsx`) mitgegeben wird. Die Debriefing-Seite filtert `spielerProtokoll` beim Lesen auf die eigene `sitzung.eigeneId` - keine getrennte Speicherung je Spieler, jede Person sieht dieselbe geteilte Liste, nur anders gefiltert. Live mit zwei echten Clients verifiziert: Übungsleitung sichtet Patient B-01, Spieler sichtet Patient B-02 - jede Person sieht in "Mein Einsatz" ausschließlich die eigene Zeile mit korrektem Patientenbezug, nie die der anderen Person, keine Konsolenfehler. |
| Zugführer-Ansicht (`DPS-0.8.1.0`) | Erster Baustein der zweiten Führungsebenen-Epoche (→ ROADMAP.md, Baustein 6): eine neue Rolle im bestehenden `Fuehrungsrolle`-Modell bekommt jetzt eine eigene Ansicht. Vorarbeit dazu war eine Recherche in FwDV 100 und dem MANV-Konzept Kreis Steinfurt - real führt dort der Zugführer nur Bereitstellungsraum und Rettungsmittelhalteplatz, die Leitung des EA Medizinische Rettung liegt beim System LNA/OrgL RD. Für dieses Übungskonzept übernimmt der Zugführer bewusst abweichend die Leitung des gesamten Abschnitts Medizinische Rettung (→ `domain.zugfuehrend`, `istZugfuehrend()`) - exakter Rollentreffer statt Rangvergleich, damit OrgL RD/LNA diese Ansicht später nicht einfach erben, sondern eigene, fachlich spezialisierte Ebenen bekommen. Ein Zugführer sieht beim Betreten des Einsatzes statt der normalen Abschnittsliste eine neue, bewusst schlankere `ZugfuehrerSeite.tsx`: Kartenansicht (oder ohne Geodaten eine kartenlose Kacheln-Rückfallebene ohne die sonst passiv sichtbare Kräfte-/Fahrzeugliste, → `ui.abschnittekurzuebersicht`) bleibt direkt sichtbar, aber anders als beim vollen Gesamtlagebild der Regie keine dauerhafte Kennzahlenleiste. Fahrzeuge, Kräfte und Kennzahlen muss der Zugführer stattdessen aktiv abfragen - drei Knöpfe blenden den jeweiligen Stand sofort ein (reine `useState`-Steuerung, keine Zeitkosten, keine neue Reducer-Aktion), passend zur realen Meldepflicht einer Führungskraft, sich das Lagebild aktiv zu verschaffen statt es automatisch vorgesetzt zu bekommen. Live mit zwei Clients verifiziert: Übungsleitung weist die Führungsrolle Zugführer zu, der Zugführer-Client sieht sofort die neue Ansicht statt der normalen Abschnittsliste, alle drei Abfrage-Knöpfe blenden ihren Inhalt erst nach Klick ein. |
...
| Gruppenführer-Ansicht, Teil 1: Grundgerüst (`DPS-0.8.2.0`) | Erster Baustein der dritten Führungsebene, auf Wunsch direkt im Anschluss an die Zugführer-Ebene begonnen. Ein Gruppenführer führte bisher ohne jede eigene Ansicht - dieselbe `Abschnittsleiste`/`PatientSeite`-Sicht wie ein einfacher Spieler, nur mit den bereits bestehenden Toast-Benachrichtigungen für Zelt-/Abschnitt-Führen-Befehle obendrauf (unverändert, laufen unabhängig von der aktuellen Seite weiter). Neue exakte Rollenprüfung `istGruppenfuehrend()` (→ `domain.gruppenfuehrend`, spiegelt `istZugfuehrend()` - kein Rangvergleich, ein Zugführer erbt diese schmalere Ansicht nicht). Neue Seite `GruppenfuehrerSeite.tsx` (→ `ui.gruppenfuehrerseite`) als neuer Startbildschirm: zeigt "Meine Gruppe" - alle per `gruppeVon()` zugewiesenen Fahrzeuge mit Typ, aktuellem Abschnitt und Besatzungsnamen, ein Klick öffnet wie bei Regie/Zugführer die gewohnte Abschnitt-Detailsicht. Bewusst deutlich schmaler als die Zugführer-Ansicht - ein Gruppenführer führt real nur seine eigene Gruppe, nicht die ganze Einsatzstelle. `EinsatzSeite.tsx` bekommt einen dritten Übersicht-Zweig neben Regie/Zugführer. `tsc`/Lint/volle Testsuite grün (3 neue Tests für `istGruppenfuehrend`). **Ungeprüfter Stand:** mangels Supabase-Zugangsdaten in dieser Sandbox nur ein Boot-Smoke-Test möglich, kein echter Mehrspieler-Testlauf mit einem Gruppenführer-Client. |
| Wegstrecken: 60m-Mindestabstand entfernt (`DPS-0.8.1.17`) | Nutzerfeedback: "Ich bekomme weiterhin nicht bei allen Verknüpfungen eine Route weder angezeigt auf Karte noch in der Liste." Ursache: `KartenOrtePanel.tsx` (Sidebar-Liste) und die Distanz-Tooltip-Anzeige in `Lagekarte.tsx` filterten Wegstrecken unter 60m komplett heraus - ein Überbleibsel aus der Zeit der automatischen Verknüpfung (→ `DPS-0.8.1.4`), als jedes Nachbarpaar automatisch eine Route bekam und eine Schwelle nötig war, um die Liste nicht mit trivialen Kurzverbindungen zu überladen. Seit `DPS-0.8.1.13` entsteht aber jede Route ausschließlich durch eine bewusste Führungsentscheidung des Zugführers - die Schwelle versteckte seitdem genau die kurzen, aber absichtlich angelegten Verbindungen vollständig aus der Liste (und ihre Distanzbeschriftung von der Karte). Beide Filter ersatzlos entfernt: `KartenOrtePanel.tsx` zeigt jetzt ausnahmslos alle `state.routen`, die Polylinie auf der Karte trägt immer ihre Distanzbeschriftung. `tsc`/Lint/volle Testsuite weiterhin grün. |
| Baumenü: Scrollen komplett von Leaflet entkoppelt (`DPS-0.8.1.16`) | Zweite Nachfolge-Korrektur: die stabilisierten Referenzen aus `DPS-0.8.1.15` verhindern zwar unnötige `instance.update()`-Aufrufe durch React-Renders, das `maxHeight={240}`-Prop auf dem Leaflet-`Popup` macht aber weiterhin Leaflets eigenen internen Inhalts-Wrapper (`_contentNode`) zum scrollbaren Element (über die von Leaflet selbst verwaltete Klasse `leaflet-popup-scrolled`) - jeder verbleibende `update()`-Aufruf (z. B. beim ersten Öffnen oder ein auf einem echten Touch-Gerät nicht auszuschließender, hier nicht reproduzierbarer weiterer Auslöser) vermisst diesen Wrapper neu und setzt `scrollTop` zurück. Behoben durch vollständige Entkopplung: `maxHeight` wird nicht mehr an das Leaflet-Popup übergeben, stattdessen bekommt eine neue, rein React-verwaltete `<div className="lagekarte-bau-menue-liste">` ihr eigenes `max-height`/`overflow-y: auto` per CSS - ein gewöhnliches, von Leaflet nie angefasstes Element. Mit einer isolierten Nachstellung (echte `leaflet`/`react-leaflet`-Pakete, Popup mit erzwungenem `instance.update()` alle 500ms als Worst-Case) bestätigt: die neue Variante hält `scrollTop` auch dabei stabil, die alte `maxHeight`-Variante setzt ihn weiterhin zurück. `tsc`/Lint/volle Testsuite weiterhin grün. |
| Baumenü: Scrollposition sprang nach dem ersten Tick sofort zurück (`DPS-0.8.1.15`) | Nachfolge-Korrektur zu `DPS-0.8.1.14`: die Reload-Schleife war behoben, aber "Nach Scroll springt Baumenü sofort auf Anfang, keine Auswahl der unten stehenden Punkte möglich" zeigte dieselbe Instabilitäts-Falle an einer zweiten Stelle. React-leaflets `createDivOverlayComponent` ruft in einem eigenen `useEffect` `instance.update()` auf, sobald sich die Objekt-Referenz von `props.children` ändert - das Popup bekam bisher bei jedem Render (jeder Simulations-Takt, alle 500ms) frisch aus JSX gebautes `children`. `instance.update()` ruft intern Leaflets `_updateLayout()` auf, die kurzzeitig `container.style.height = ''` setzt, um die Inhaltshöhe neu zu messen - dabei passt der Inhalt kurz wieder vollständig ins scrollbare Element, der Browser setzt `scrollTop` in diesem Moment auf 0 zurück, die Höhe wird danach zwar korrekt wiederhergestellt, aber die Scrollposition bleibt verloren. Behoben, indem sowohl `nochOffeneAbschnitte` (jetzt `useMemo` über `state.flaechen`/`state.flaechenBefehle` - beide zwischen Ticks referenzstabil, da der `tick`-Fall im Reducer sie nie anfasst) als auch der komplette Popup-Inhalt (`bauMenuInhalt`, `useMemo` über `bauMenuPosition`/`nochOffeneAbschnitte`) referenzstabil gehalten werden - beide Hooks vor dem frühen Return der Komponente. `tsc`/Lint/volle Testsuite weiterhin grün. |
| Baumenü: Popup schloss und öffnete sich alle 500ms neu (`DPS-0.8.1.14`) | Nutzerfeedback: "Das Baumenü hat Probleme, es lädt immer neu, somit kann ich hier nicht scrollen." Ursache: `Lagekarte.tsx` übergab dem Leaflet-`Popup` die Position inline als `position={[bauMenuPosition.lat, bauMenuPosition.lon]}` - ein bei jedem Render neu erzeugtes Array-Literal. React-leaflets Popup-Lebenszyklus vergleicht `position` per Referenz in einer `useEffect`-Abhängigkeitsliste; jede neue Array-Instanz sah wie eine echte Positionsänderung aus, worauf die Lebenszyklus-Logik das Popup vollständig von der Karte entfernte und sofort wieder öffnete. Da die Simulationsuhr (→ `state.uhr`) alle 500ms tickt und jede Komponente re-rendert, die `useSimulation()` liest, geschah das im laufenden Betrieb ununterbrochen - jeder Scrollversuch im Popup-Inhalt wurde durch das nächste Neuöffnen sofort zunichtegemacht. Behoben durch zwei mit `useMemo`/`useCallback` stabilisierte Referenzen (`bauMenuLatLng`, `bauMenuEventHandlers`, vor dem frühen Return der Komponente, da Hooks nie bedingt aufgerufen werden dürfen), die sich nur noch bei einer echten Änderung von `bauMenuPosition` ändern. Zusätzlich `maxHeight={240}` auf dem Popup, damit eine lange Abschnittsliste (bis zu neun Einträge) unabhängig vom Reload-Bug per `overflow: auto` scrollbar ist statt über den Kartenrahmen hinauszulaufen (Leaflet setzt die dafür nötige `leaflet-popup-scrolled`-Klasse nur bei gesetztem `maxHeight`). `tsc`/Lint/volle Testsuite grün (Baseline: die 2 bekannten `turnAnbieter.test.ts`-Umgebungsausfälle) - eine eigene Komponenten-Testsuite existiert in diesem Projekt bisher nicht, die Reducer-/Domain-Logik war von diesem rein UI-seitigen Fehler nicht betroffen. |
...
| Meldebuch: Zugführer fragt Gruppenführer per Funk statt Live-Daten (`DPS-0.8.1.9`) | Auf ausdrücklichen Wunsch: der Zugführer sah in den Tabs Fahrzeuge/Kräfte/Kennzahlen bislang sofort den echten, live berechneten Stand - real muss er sich diesen Stand per Funk bei seinen Gruppenführern aktiv erfragen. Die App kann den Inhalt eines echten Funkspruchs nicht kennen, deshalb ersetzt eine neue, wiederverwendbare `Meldebuch`-Komponente (→ `ui.meldebuch`) die drei bisherigen Live-Panels (`FahrzeugStatusPanel.tsx`, `KraefteStatusPanel.tsx`, `Kennzahlenleiste`-Nutzung in der Zugführer-Ansicht) durch ein Freitext-Protokoll je Bereich: der Zugführer tippt selbst ein, was er über Funk erfahren hat. Neues, nur wachsendes State-Feld `state.meldebuch: MeldebuchEintrag[]` (`id`, `bereich: MeldebuchBereich`, `text`, `zeitSek`, `spielerId`, → `modell.meldebucheintrag`) über eine neue Reducer-Aktion `meldebuchEintragen` (Leerstring-Guard, `text.trim()`) - an allen fünf Schnappschuss-Touchpoints sowie den drei `SimulationProvider.tsx`-Stellen ergänzt, damit es korrekt zwischen allen Clients synchron bleibt. `Kennzahlenleiste.tsx` selbst bleibt unverändert (die Regie braucht dort weiterhin echte Zahlen für die Aufsicht) - nur die Zugführer-Ansicht verwendet sie nicht mehr. `FahrzeugStatusPanel.tsx`/`KraefteStatusPanel.tsx` sind jetzt ungenutzt und gelöscht. Die Einträge bleiben bis zum Debriefing erhalten: ein neuer Abschnitt "Meldebuch des Zugführers" zeigt dort alle Einträge chronologisch mit Bereichs-Tag, Autor:in und Zeitstempel - zum Abgleich mit der tatsächlichen Lage in der Haupttabelle, genau der pädagogische Witz der Übung. Live mit zwei Clients verifiziert: alle drei Tabs starten leer ("Noch keine Meldung eingetragen."), ein eingetragener Text bleibt beim Tab-Wechsel und bei erneutem Öffnen sichtbar, die Regie-Kennzahlenleiste im Gesamtlagebild bleibt währenddessen unverändert live (echte SK-Zahlen, keine Meldebuch-Texte), nach Einsatzende zeigt das Debriefing alle drei eingetragenen Meldungen mit korrektem Bereich, Autor:in und Zeitstempel, keine Konsolenfehler. |
| Zugführer-Ansicht als Seitenleisten-Ansichtsmenü (`DPS-0.8.1.8`) | Auf ausdrücklichen Wunsch: die Zugführer-Ansicht (→ `ui.zugfuehrerseite`) optisch an das Gesamtlagebild der Regie angleichen - mit denselben, schon vorhandenen Menüpunkten (Kacheln, Karte, Fahrzeuge, Kräfte, Kennzahlen) statt der bisherigen Kombination aus gestapelter Karte+Kacheln und einer separaten Abfrage-Tab-Leiste darunter. `RegieMenueIcons.tsx` wird zu `AnsichtsmenueIcons.tsx` (Komponente `AnsichtsmenueIcon`, Anker `ui.regiemenueicons` → `ui.ansichtsmenueicons`) mit drei neuen Icons (Fahrzeuge, Kräfte, Kennzahlen) - jetzt von beiden Seitenleisten-Ansichtsmenüs geteilt statt Regie-exklusiv. Die zugrundeliegenden CSS-Klassen sind ebenfalls generisch umbenannt (`.regie-seitenleiste*`/`.regie-menue*`/`.gesamtlagebild-flaeche`/`.gesamtlagebild-haupt` → `.ansichtsmenue-seitenleiste*`/`.ansichtsmenue*`/`.ansicht-flaeche`/`.ansicht-haupt`), ein reiner Rename ohne Verhaltensänderung für das Gesamtlagebild. `ZugfuehrerSeite.tsx` bekommt dieselbe Ein-/Ausklapp-Seitenleiste mit `role="tablist"` wie die Regie - Kacheln bleiben wie zuvor der einzige Weg in die Abschnitt-Detailsicht, die Lagekarte steht jetzt als eigene Ansicht daneben statt zusätzlich obendrüber gestapelt; Fahrzeuge/Kräfte/Kennzahlen bleiben bewusst nur nach aktivem Klick sichtbar (keine dauerhafte Kennzahlenleiste, passend zur realen Meldepflicht). Alte `.zf-abfrage*`-CSS-Regeln entfernt. Live verifiziert: Zugführer- und Regie-Seitenleiste sehen optisch identisch aus (gleiches Layout, gleiche Icons, gleiches aktiv/eingeklappt-Verhalten), alle fünf Zugführer-Ansichten lassen sich über das Menü erreichen, Ein-/Ausklappen funktioniert, ein Klick auf eine Kachel öffnet weiterhin korrekt die Abschnitt-Detailsicht mit funktionierendem "← Übersicht"-Rücksprung, Regie-Ansicht nach dem Rename unverändert funktionsfähig, keine Konsolenfehler. |
| Lagekarte, Teil 3: Bauen per Kartenklick statt sofortiger Platzhalter (`DPS-0.8.1.7`) | Auf ausdrücklichen Wunsch: eine Übung soll immer nur mit der Schadensstelle sichtbar beginnen, alles andere muss erst gebaut werden, bevor es auf der Karte erscheint - bisher zeigte die Lagekarte alle acht weiteren Abschnitte von Anfang an als schematischen Platzhalter-Marker. `Lagekarte.tsx`s `sichtbarePosition()`-Helfer (→ `ui.lagekarte`) liefert für jeden `FlaechenAbschnitt` erst dann eine Position (Marker wie Route), wenn wirklich eine `PlatzierteFlaeche` existiert - am echten Mittelpunkt des gebauten Rechtecks statt am alten festen Schlüsselpunkt, Routen verschwinden entsprechend, solange eines ihrer beiden Enden noch nicht gebaut ist. Der Bauablauf selbst läuft jetzt komplett über die Karte statt über eine feste Knopfliste (→ `ui.lagekarte.bauen`): ein Kartenklick öffnet an genau dieser Stelle ein Leaflet-Popup mit den noch offenen Abschnitten, danach wie gehabt `ZeltTypAuswahl` für die Größe - aber keine sofortige Platzierung mehr. Stattdessen erscheint eine ziehbare Vorschau (Rechteck + Ziehgriff-Marker) am Klickpunkt, frei auf der Karte verschiebbar, live grün/rot eingefärbt je nach `platzierungGueltig()`; erst ein bewusstes "Bauort bestätigen" löst die bestehende Auftragstaktik-Entscheidung und den echten Bau-Countdown aus, "Abbrechen" verwirft die Vorschau folgenlos. Neue Leaflet-Popup-Dark-Theme-Overrides, tote `.baufeld-farb-knoepfe`-Regel entfernt. Live verifiziert: Szenario startet mit ausschließlich der Schadensstelle sichtbar; Kartenklick öffnet das Baumenü mit allen offenen Abschnitten; Vorschau lässt sich ziehen und färbt sich korrekt grün/rot (innerhalb/außerhalb des für die drei Zeltfarben geprüften Baufelds), "Bauort bestätigen" bleibt bei ungültiger Stelle deaktiviert; nach Bestätigung und Bauzeit erscheinen sowohl die gebaute Fläche als auch die zuvor unsichtbare Route zur Schadensstelle korrekt auf der Karte; keine Konsolenfehler. |
| Lagekarte, Teil 2: React-Leaflet + taktische Zeichen (`DPS-0.8.1.6`) | Löst das in Teil 1 vorbereitete Datenmodell ein: `Lagekarte.tsx` (→ `ui.lagekarte`) ersetzt `Kartenansicht.tsx` und `Baufeld.tsx` vollständig durch eine einzige echte Karte - React-Leaflet mit OpenStreetMap-Kacheln, zentriert auf `Szenario.geodaten.ursprung`, Maßstab ergibt sich automatisch aus echten Koordinaten statt einer separat gepflegten Distanzangabe. Jeder Schlüsselpunkt zeigt ein echtes taktisches Zeichen nach DV 102 (→ `domain.taktischezeichen`, Bibliothek `taktische-zeichen-core`: Grundzeichen `ortsfeste-stelle` für alle festen Einrichtungen, `gefahr` für die Schadensstelle, Organisation `hilfsorganisation` als nächstliegender Ersatz für Rettungsdienst) als `L.divIcon`, solange dort keine Fläche steht - danach übernimmt ein maßstabsgetreues Rechteck (`lokalMZuGeo`) dieselbe Stelle. Die Platzierung läuft wie im bisherigen Baufeld über Größe wählen → Auftragstaktik-Entscheidung ("Wie bauen?") → Countdown, nur ein echter Kartenklick statt einer Rasterzelle liefert über `geoZuLokalM` die Koordinaten für denselben Dispatch. Ein `ResizeObserver` in einer eigenen `KartenGroessenBeobachter`-Unterkomponente ruft bei jeder Container-Größenänderung `map.invalidateSize()` - unabhängig von der Ursache (Regie-Seitenleiste ein-/ausklappen, Fenstergröße, Bildschirmdrehung), ohne dass die Elternseite dafür etwas tun muss. Alle sechs Szenarien (`wohnungsbrand` und die vier `EINZELFAELLE`) bekommen dafür echte, plausible Geodaten (reale Orte im Kreis Steinfurt, keine echten Einsätze) - `busunfall-b31` hatte sie schon aus einer früheren Erweiterung. `ZugfuehrerSeite.tsx` und `GesamtlagebildSeite.tsx` (dort `interaktiv={false}`, rein lesend) zeigen jetzt beide dieselbe `Lagekarte`, die alte `hatGeodaten`-Bedingung entfällt (jetzt immer erfüllt). Live verifiziert (drei echte Clients: Übungsleitung, Zugführer, Beobachter): Zelt-/Flächenwahl bei den Behandlungszelten zeigt weiterhin beide Kataloge, eine Platzierung außerhalb der (nur für die drei Zeltfarben geprüften) Baufeld-Grenze wird korrekt abgelehnt, eine gültige Platzierung löst den Bau-Countdown mit korrekter Beschriftung aus, die Regie-Ansicht bleibt schreibgeschützt (keine Platzieren-Knöpfe) und übersteht das Einklappen der Seitenleiste ohne kaputte Karte, keine Konsolenfehler. Die OpenStreetMap-Kachelbilder selbst ließen sich in der Entwicklungsumgebung dieser Sitzung nicht laden (Netzwerk-Sandbox blockiert Bildanfragen an `tile.openstreetmap.org` aus dem Headless-Browser heraus, obwohl derselbe Host per `curl` erreichbar ist) - alle übrigen Kartenbestandteile (Marker, Routen, Zoom-Steuerung, Attribution, Flächen-Rechtecke) sind davon unberührt und funktionieren nachweislich korrekt. |
| Lagekarte, Teil 1: Datenmodell (`DPS-0.8.1.4`) | Auf ausdrücklichen Wunsch: eine echte, maßstabsgetreue Karte (OpenStreetMap) soll die bisherige schematische Kartenansicht und das abstrakte Baufeld ersetzen, mit exakter Standortplatzierung für alle Einsatzabschnitte statt nur der drei Behandlungszelte - Ein-/Ausgangssichtung dürfen jetzt ebenfalls als Zelt ausgeführt werden. Dieser erste Teil landet nur das Datenmodell, noch hinter der bisherigen Baufeld-Oberfläche. `ZeltAbschnitt` (3 Farben) → `FlaechenAbschnitt` (8 Abschnitte), `PlatzierterZelt`→`PlatzierteFlaeche`, `ZeltBefehl`→`FlaechenBefehl`, `domain/zelte.ts`→`domain/flaechen.ts` (→ `domain.flaechen`). Neuer, zu `ZELTTYPEN` paralleler Katalog `FLAECHENTYPEN` (kleine/mittlere/große markierte Fläche ohne reales Zeltprodukt, kurze Aufbauzeit statt echter Zeltbauzeit) - Ein-/Ausgangssichtung wählen aus beiden Katalogen, die übrigen drei Nicht-Zelt-Abschnitte nur aus `FLAECHENTYPEN`. `platzierungGueltig` bekommt einen `pruefeGrenzen`-Parameter (Standard `true`) - nur die drei Zelte werden gegen die enge Baufeld-Grenze geprüft, die übrigen fünf liegen oft weit außerhalb davon auf der echten Einsatzstelle. Neuer Referenzpunkt `Szenario.geodaten.ursprung`, von dem aus die gesamte Einsatzstelle in Metern gemessen wird (→ `domain.geodaten.projektion`, `geoZuLokalM`/`lokalMZuGeo`) - ersetzt zwei bisher unabhängige Koordinatensysteme durch eines und behebt damit die ursprünglich gemessene Diskrepanz (bis zu 54 % Abweichung zwischen Kartenansicht und `Route.distanzMeter`) grundsätzlich. `RouteVorlage.distanzMeter` wird optional - ohne Angabe berechnet `routenAusSzenario` die echte Distanz aus den Koordinaten (→ `domain.geodaten.haversine`), ein manueller Override bleibt für echte Abweichungen möglich. Die bisherige `eroeffneteAbschnitte`-Liste samt `abschnittEroeffnen`-Aktion entfällt. 467 Tests grün, live verifiziert (alle acht Abschnitte platzierbar, Eingangssichtung zeigt beide Kataloge, Ablage nur Flächen, Aufbau-Countdown korrekt beschriftet, keine Konsolenfehler). |
| Mikromanagement im Baufeld (`DPS-0.8.1.3`) | Kleine Ergänzung zu `DPS-0.8.1.2` auf ausdrücklichen Wunsch: Auftragstaktik (Zugführer befiehlt, Gruppenführer baut) soll der Normalfall bleiben, der Zugführer aber die grundsätzliche Möglichkeit haben, selbst einzugreifen - genau die reale Spannung, die Auftragstaktik als Führungsprinzip bewusst einschränkt. Bewusst kein Schalter oder fester Modus, sondern eine Entscheidung pro Platzierung: sitzt mindestens ein Gruppenführer in der Sitzung, fragt `Baufeld.tsx` (→ `ui.baufeld.befehl`) nach jeder Standortwahl "Wie bauen?" mit zwei Knöpfen - "Befehl an Gruppenführer geben" oder "Selbst bauen (Mikromanagement)", ohne gespeicherte Präferenz (kein neues State-/Reducer-Feld). "Selbst bauen" nutzt denselben Pfad wie der bisherige Kein-Gruppenführer-Rückfall - kein Befehl, keine Benachrichtigung; "Befehl geben" verhält sich exakt wie das aus `DPS-0.8.1.2` bekannte Verhalten (bei mehreren Gruppenführern weiterhin mit Zielperson-Auswahl). Live mit drei echten Clients verifiziert: Entscheidung erscheint nach jeder Standortwahl; "Selbst bauen" gewählt → Zelt entsteht direkt beim Zugführer ohne Benachrichtigung; "Befehl geben" beim zweiten Zelt gewählt → wie gehabt eine Benachrichtigung beim Gruppenführer, keine Konsolenfehler. |
...
| `domain.flaechen` | [`src/domain/flaechen.ts:10`](src/domain/flaechen.ts#L10) | Reale Zeltgrößen und markierte Flächen für die Einsatzstelle |
| `domain.fuehrung` | [`src/domain/fuehrung.ts:5`](src/domain/fuehrung.ts#L5) | Rangfolge und Prüfung der Führungsrolle |
| `domain.geodaten` | [`src/domain/geodaten.ts:64`](src/domain/geodaten.ts#L64) | Verlegungsdauer aus echter Distanz statt Pauschale |
| `domain.geodaten.haversine` | [`src/domain/geodaten.ts:22`](src/domain/geodaten.ts#L22) | Echte Distanz zwischen zwei Schlüsselpunkten |
| `domain.geodaten.projektion` | [`src/domain/geodaten.ts:42`](src/domain/geodaten.ts#L42) | Lat/Lon ↔ lokale Meter, ein einziges Koordinatensystem für die Einsatzstelle |
| `domain.gewicht` | [`src/domain/dosierung.ts:337`](src/domain/dosierung.ts#L337) | Körpergewicht - hinterlegt oder geschätzt |
| `domain.gruppenfuehrend` | [`src/domain/fuehrung.ts:78`](src/domain/fuehrung.ts#L78) | Der Gruppenführer sieht eine Übersicht seiner eigenen Gruppe |
| `domain.gruppenfuehrerliste` | [`src/domain/fuehrung.ts:124`](src/domain/fuehrung.ts#L124) | Wer als Gruppenführer in der Sitzung mitspielt |
| `domain.gruppenmitglieder` | [`src/domain/fuehrung.ts:150`](src/domain/fuehrung.ts#L150) | Die Personen einer Gruppe, ohne den Gruppenführer selbst |
| `domain.gruppevon` | [`src/domain/fuehrung.ts:137`](src/domain/fuehrung.ts#L137) | Die einem Gruppenführer zugewiesenen Fahrzeuge (→ `modell.gruppe`) |
...
| `domain.staerkemeldung` | [`src/domain/fuehrung.ts:174`](src/domain/fuehrung.ts#L174) | Reale Stärkemeldung einer Fahrzeugbesatzung |
...
| Anker | Datei | Bedeutung |
| --- | --- | --- |
| `szenarien.busunfall` | [`src/domain/szenarien.ts:21`](src/domain/szenarien.ts#L21) | Zehn Patienten als Vorlage für eigene Szenarien |
| `szenarien.liste` | [`src/domain/szenarien.ts:14`](src/domain/szenarien.ts#L14) | Die Übungsszenarien - hier neue Lagen und Patienten anlegen |
...
| `ui.abschnittekurzuebersicht` | [`src/components/AbschnitteKurzuebersicht.tsx:13`](src/components/AbschnitteKurzuebersicht.tsx#L13) | Kacheln ohne Kräfte-/Fahrzeugdetails |
...
| `ui.abschnittfuehrenbefehl` | [`src/components/AbschnittFuehrenBefehlBenachrichtigung.tsx:6`](src/components/AbschnittFuehrenBefehlBenachrichtigung.tsx#L6) | Benachrichtigung: der Zugführer befiehlt der Gruppe, einen Abschnitt zu führen |
| `ui.abschnittsleiste` | [`src/components/Abschnittsleiste.tsx:7`](src/components/Abschnittsleiste.tsx#L7) | Reiter mit der Belegung je Abschnitt |
...
| `ui.debriefing` | [`src/pages/DebriefingSeite.tsx:38`](src/pages/DebriefingSeite.tsx#L38) | Auswertung nach dem Einsatz |
...
| `ui.fahrzeugverlegung` | [`src/components/FahrzeugVerlegung.tsx:11`](src/components/FahrzeugVerlegung.tsx#L11) | Fahrzeuge zwischen Abschnitten verlegen - nur mit Zugführer-Rang |
...
| `DPS-0.8.1.15` | Baumenü: Scrollposition sprang sofort zurück, weil react-leaflets `createDivOverlayComponent` bei jeder neuen `children`-Referenz (jeder Simulations-Takt) `instance.update()` aufruft, was Leaflets `_updateLayout()` und damit einen `scrollTop`-Reset auslöst; behoben mit `useMemo`-stabilisierten `nochOffeneAbschnitte` (über `state.flaechen`/`state.flaechenBefehle`) und `bauMenuInhalt` |
| `DPS-0.8.1.14` | Baumenü-Popup schloss und öffnete sich alle 500ms neu (Reducer-Tick, → `state.uhr`), weil `position={[lat, lon]}` bei jedem Render ein neues Array war und react-leaflets Popup-Lebenszyklus das per Referenz statt per Wert vergleicht; behoben mit `useMemo`/`useCallback`-stabilisierten `bauMenuLatLng`/`bauMenuEventHandlers` + `maxHeight={240}` für echtes internes Scrollen bei langer Abschnittsliste |
...
| `DPS-0.8.1.9` | Meldebuch: Zugführer trägt Fahrzeuge/Kräfte/Kennzahlen jetzt selbst per Funk-Rückfrage ein statt Live-Daten zu sehen - neue `Meldebuch`-Komponente + `state.meldebuch`/`meldebuchEintragen`, ersetzt `FahrzeugStatusPanel.tsx`/`KraefteStatusPanel.tsx`/die Zugführer-Nutzung von `Kennzahlenleiste` (bleibt für Regie live unverändert); Einträge erscheinen chronologisch im Debriefing |
| `DPS-0.8.1.8` | Zugführer-Ansicht als Seitenleisten-Ansichtsmenü: `ZugfuehrerSeite.tsx` optisch an `ui.gesamtlagebild` angeglichen (fünf Ansichten Kacheln/Karte/Fahrzeuge/Kräfte/Kennzahlen im Ein-/Ausklapp-Seitenleisten-Menü statt gestapelter Karte + Abfrage-Tab-Leiste); `RegieMenueIcons.tsx`→`AnsichtsmenueIcons.tsx` (`AnsichtsmenueIcon`, + 3 neue Icons), CSS generisch umbenannt (`.regie-seitenleiste*`/`.regie-menue*`/`.gesamtlagebild-flaeche`/`.gesamtlagebild-haupt` → `.ansichtsmenue-seitenleiste*`/`.ansichtsmenue*`/`.ansicht-flaeche`/`.ansicht-haupt`), jetzt von beiden Seiten geteilt; alte `.zf-abfrage*`-Regeln entfernt |
| `DPS-0.8.1.7` | Lagekarte, Teil 3: nur die Schadensstelle startet sichtbar, jeder weitere Abschnitt (Marker wie Route) erscheint erst nach dem Bau (→ `sichtbarePosition()`); Bauablauf jetzt per Kartenklick → Leaflet-Popup-Baumenü → `ZeltTypAuswahl` → ziehbare, live grün/rot validierte Vorschau → "Bauort bestätigen" löst Auftragstaktik/Countdown aus (→ `ui.lagekarte.bauen`); alte feste Knopfliste + Sofortplatzierung entfernt |
| `DPS-0.8.1.6` | Lagekarte, Teil 2: `Lagekarte.tsx` (→ `ui.lagekarte`) ersetzt `Kartenansicht.tsx` + `Baufeld.tsx` vollständig - React-Leaflet mit OpenStreetMap-Kacheln, taktische Zeichen nach DV 102 als `L.divIcon` je Schlüsselpunkt (→ `domain.taktischezeichen`), Platzierung per echtem Kartenklick statt Rasterzelle; neue `domain/taktischeZeichen.ts`; `wohnungsbrand` + die vier `EINZELFAELLE` bekommen echte Geodaten (→ `einzelfallGeodaten()`); `ZugfuehrerSeite.tsx`/`GesamtlagebildSeite.tsx` (dort `interaktiv={false}`) auf `Lagekarte` umgestellt, `hatGeodaten`-Bedingungen entfernt; `.karten-rahmen` von `aspect-ratio` auf feste Höhe, tote SVG-CSS entfernt |
| `DPS-0.8.1.5` | Zelt-/Flächenwahl auch bei den Behandlungszelten: `ZeltTypAuswahl.tsx`s `katalogeFuer()` bietet jetzt bei den drei Behandlungszelten und Ein-/Ausgangssichtung immer beide Kataloge (echtes Zelt oder reine Fläche), Ablage/Bereitstellungsraum/Transport bleiben bei der reinen Fläche |
| `DPS-0.8.1.4` | Lagekarte, Teil 1 (Datenmodell): `ZeltAbschnitt`→`FlaechenAbschnitt` (8 statt 3 Abschnitte), `PlatzierterZelt`→`PlatzierteFlaeche`, `ZeltBefehl`→`FlaechenBefehl`, `domain/zelte.ts`→`domain/flaechen.ts`; neuer `FLAECHENTYPEN`-Katalog für reine Flächen ohne Zeltprodukt; `platzierungGueltig` mit `pruefeGrenzen`-Parameter; neuer `Szenario.geodaten.ursprung` + `haversineMeter`/`geoZuLokalM`/`lokalMZuGeo` (→ `domain.geodaten.projektion`) - ein einziges Koordinatensystem für die ganze Einsatzstelle statt zweier getrennter; `RouteVorlage.distanzMeter` optional, wird sonst aus echten Koordinaten berechnet; `eroeffneteAbschnitte`/`abschnittEroeffnen` entfernt |
| `DPS-0.8.1.3` | Mikromanagement im Baufeld: sitzt ein Gruppenführer in der Sitzung, fragt `Baufeld.tsx` (→ `ui.baufeld.befehl`) nach jeder Standortwahl "Wie bauen?" - Befehl an Gruppenführer oder selbst bauen (Mikromanagement); keine feste Betriebsart oder gespeicherte Präferenz, sondern eine bewusste Wahl pro Zelt, kein neues State-/Reducer-Feld |
| `DPS-0.8.1.2` | Zeltbefehl: Auftragstaktik statt Direktbau (→ `modell.zeltbefehl`) - der Zugführer legt Zeltgröße/Position im Baufeld fest, sitzt ein Gruppenführer in der Sitzung, löst das einen Befehl statt einer direkten Platzierung aus; die Ausführung (echter Bau-Countdown) liegt beim Gruppenführer (→ `ui.zeltbefehlbenachrichtigung`, Delegationsanfrage-Muster); ohne Gruppenführer baut der Zugführer weiterhin direkt |
| `DPS-0.8.1.1` | Baufeld: Zeltplatzierung mit realen DRK-Zeltgrößen (SG20-50, → `domain.zelte`), maßstabsgetreu in einem neuen, von der lat/lon-Kartenansicht unabhängigen Baufeld; echter Zeit-Countdown beim Aufbau; geteiltes Flächenbudget mit Fahrzeugen (weiche Warnung); Zeltplatzierung fungiert als Abschnitt-Eröffnung für die drei Zelte, einfacher Eröffnen-Knopf für die übrigen fünf Abschnitte (Schadensstelle bleibt immer offen) |
| `DPS-0.8.1.0` | Zugführer-Ansicht: neue `ZugfuehrerSeite.tsx` (Karte/Kacheln-Rückfallebene ohne passive Kräfte-/Fahrzeugliste, plus drei Abfrage-Knöpfe Fahrzeuge/Kräfte/Kennzahlen) für Spieler mit Führungsrolle Zugführer (`istZugfuehrend()`, exakter Rollentreffer), erster Baustein der Zugführer-Epoche (`DPS-0.8.1.x`) |
| `DPS-0.8.0.15` | Private Spieler-Statusansicht "Mein Einsatz": neue Debriefing-Sektion mit den eigenen Handlungen einer Person - neues `state.spielerProtokoll`, gefüllt aus fünf Reducer-Aktionen (Diagnostik/Sichtung/Maßnahme/Verlegung/Rettungsmaterial) plus Team-Abschlüssen (Narkose, Rettung), gefiltert nach `sitzung.eigeneId` beim Lesen |
