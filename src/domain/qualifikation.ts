 ...
 *
 * Zwei unabhängig einstellbare Schwellen je Maßnahme (→ `domain.massnahmenrechte`):
 * Durchführung und Delegation müssen nicht an derselben Stufe hängen - z. B.
 * darf NotSan eine Maßnahme selbst durchführen, aber nur Notarzt darf sie an
 * eine niedrigere Stufe delegieren. Die Übungsleitung stellt beides vor dem
 * Start ein; ohne Anpassung startet jede Maßnahme mit ihrer Katalog-Stufe für
 * beides (→ `domain.massnahmenrechte`, `standardMassnahmenrechte`).
 */
export interface MassnahmeRecht {
  /** Mindeststufe, um die Maßnahme selbst durchzuführen. */
  qualifikation: Qualifikation;
  /** Mindeststufe, um die Maßnahme für einen Patienten zu delegieren. */
